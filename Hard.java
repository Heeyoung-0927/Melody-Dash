 ...
 * June 13th 2024
 * This program displays the hard mode of Melody Dash
 */
package graphics;
...
public class Hard implements ActionListener {
	final int WIDW = 1280; // Window width
	final int WIDH = 650; // Window height
	final int TILEWIDTH = WIDW / 8; // Tile width
	final int TILEHEIGHT = 100; // Tile height
	final int TILESPEED = 20; // Speed of tiles falling
	final int SPAWNINTERVAL = 20; // Period of creating one tile
	...
	GraphicsConsole gc = new GraphicsConsole(WIDW, WIDH, "Hard Mode");
	// Attributes
	...
	private void update() {
		...
		checkTileHit(KeyEvent.VK_A, 0);
		checkTileHit(KeyEvent.VK_S, 1);
		checkTileHit(KeyEvent.VK_D, 2);
		checkTileHit(KeyEvent.VK_F, 3);
		checkTileHit(KeyEvent.VK_H, 4);
		checkTileHit(KeyEvent.VK_J, 5);
		checkTileHit(KeyEvent.VK_K, 6);
		checkTileHit(KeyEvent.VK_L, 7);
	}
		...
		if (gc.isKeyDown(key)) {
			for (int i = 0; i < tiles.size(); i++) {
				...
				if (tile.x == lane * TILEWIDTH && (tile.y + tile.height) >= YLINE - TILESPEED) {
					...
					} else if (Math.abs(tileCenter - YLINE) <= 18) {
						score += 3;
						perfect = false;
						good = true;
						bad = false;
						if (!gt.isRunning()) gt.restart();
					} else if (Math.abs(tileCenter - YLINE) <= 52) {
						score += 1;
						perfect = false;
						good = false;
						bad = true;
	...
	private void gameOver() {
        ...
        gc.drawString("Press M for Menu", 550, 400);
        while (true) {
