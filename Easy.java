 ...
 * June 13th 2024
 * This program displays the easy mode of Melody Dash
 */
package graphics;
...
public class Easy implements ActionListener {
	final int WIDW = 700; // Window width
	final int WIDH = 650; // Window height
	final int TILEWIDTH = WIDW / 4; // Tile width
	final int TILEHEIGHT = 100; // Tile height
	final int TILESPEED = 5; // Speed of tiles falling
	final int SPAWNINTERVAL = 50; // Period of creating one tile
	...
	GraphicsConsole gc = new GraphicsConsole(WIDW, WIDH, "Easy Mode");
	// Attributes
	...
	private void update() {
		...
		checkTileHit(KeyEvent.VK_D, 0);
		checkTileHit(KeyEvent.VK_F, 1);
		checkTileHit(KeyEvent.VK_H, 2);
		checkTileHit(KeyEvent.VK_J, 3);
	}
	...
	private void gameOver() {
        ...
        gc.drawString("Press M for Menu", 255, 440);
        while (true) {
